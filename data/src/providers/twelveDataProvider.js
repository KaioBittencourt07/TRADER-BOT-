...
function deriveTechnical(values) {
  ...
  const structureRaw = range > 0 ? ((last - recentLow) / range) * 2 - 1 : 0;
  ...
  const signs = [trend, momentum, structure].filter(v => Math.abs(v) >= 0.15).map(v => Math.sign(v));
  const dominant = signs.length ? Math.sign(signs.reduce((a, b) => a + b, 0)) : 0;
  const confirmations = signs.filter(v => v === dominant).length + (last > sma5 === last > sma12 ? 1 : 0);
